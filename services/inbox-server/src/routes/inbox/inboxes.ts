...
app.post("/:id/routing", requireAdmin, async (c) => {
	const row = await rawDb.first<{ id: string; email: string }>("SELECT id, email FROM inboxes WHERE id = ? LIMIT 1", [
		c.req.param("id"),
	]);
	...
	await rawDb.run("UPDATE inboxes SET routing_configured = ?, routing_error = ? WHERE id = ?", [
		routing.configured ? 1 : 0,
		routing.error ?? null,
		c.req.param("id"),
	]);
...
app.post("/:id/members", requireAdmin, zValidator("json", z.object({ userId: z.string() })), async (c) => {
	...
	await rawDb.run("INSERT OR IGNORE INTO inbox_members (inbox_id, user_id) VALUES (?, ?)", [
		c.req.param("id"),
		userId,
	]);
...
app.delete("/:id/members/:userId", requireAdmin, async (c) => {
	await rawDb.run("DELETE FROM inbox_members WHERE inbox_id = ? AND user_id = ?", [
		c.req.param("id"),
		c.req.param("userId"),
	]);
	return c.json({ deleted: true });
