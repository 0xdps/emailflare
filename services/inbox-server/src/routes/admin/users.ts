...
app.get("/", requireSession, requireSuperAdmin, async (c) => {
	const { rows } = await rawDb.query("SELECT id, name, email, role, created_at FROM users ORDER BY created_at ASC");
	return c.json(rows);
});
...
app.delete("/:id", requireSession, requireSuperAdmin, async (c) => {
	...
	const user = await rawDb.first<{ id: string; role: string }>("SELECT id, role FROM users WHERE id = ? LIMIT 1", [
		id,
	]);
...
app.patch(
	...
	async (c) => {
		...
		const user = await rawDb.first<{ id: string; role: string }>(
