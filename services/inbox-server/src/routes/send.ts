...
app.post("/", zValidator("json", sendSchema), async (c) => {
	...
	for (const recipient of toList) {
		...
		if (body.listId && publicOrigin) {
			...
			await rawDb.run("INSERT INTO unsubscribe_tokens (token, email, list_id, created_at) VALUES (?, ?, ?, ?)", [
				token,
				recipient.toLowerCase(),
				body.listId,
				now,
		...
		try {
			...
			const out = await sendWithLog(
				{
					deliver: async (msg) =>
						isTest ? { id: generateId() } : sendEmail(msg, env.CF_API_TOKEN, env.CF_ACCOUNT_ID),
	...
	if (successCount > 0) {
		await rawDb.run("UPDATE api_keys SET last_used_at = ?, send_count = send_count + ? WHERE id = ?", [
			now,
			successCount,
			apiKey.keyId,
		]);
