...
async function appliedMigrations(): Promise<Set<string>> {
	const result = await mesaDb.query("SELECT name FROM schema_migrations ORDER BY name ASC", []);
...
export async function runMigrations(): Promise<void> {
	...
	for (const file of files) {
		...
		try {
			await mesaDb.exec("INSERT INTO schema_migrations (name, applied_at) VALUES (?, ?)", [
				file,
