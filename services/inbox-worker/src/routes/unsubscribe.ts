...
//   POST /v1/unsubscribe?token=…  → one-click unsubscribe (RFC 8058)
//
...
async function performUnsubscribe(env: HonoEnv['Bindings'], token: string): Promise<{ email: string } | null> {
  const { db, suppressions } = makeDb(env.DB);
  const rec = await db.query<{ token: string; email: string; list_id: string | null }>(
