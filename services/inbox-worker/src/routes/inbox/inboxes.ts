...
app.post("/:id/routing", requireAdmin, async (c) => {
	const row = await c.env.DB.prepare("SELECT id, email FROM inboxes WHERE id = ? LIMIT 1")
		.bind(c.req.param("id"))
		.first();
	...
	await c.env.DB.prepare("UPDATE inboxes SET routing_configured = ?, routing_error = ? WHERE id = ?")
		.bind(routing.configured ? 1 : 0, routing.error ?? null, c.req.param("id"))
		.run();
...
app.put("/:id", requireAdmin, zValidator("json", inboxSchema.partial()), async (c) => {
	...
	await c.env.DB.prepare(`UPDATE inboxes SET ${set} WHERE id = ?`)
...
app.post("/:id/members", requireAdmin, zValidator("json", z.object({ userId: z.string() })), async (c) => {
	...
	await c.env.DB.prepare("INSERT OR IGNORE INTO inbox_members (inbox_id, user_id) VALUES (?, ?)")
...
app.delete("/:id/members/:userId", requireAdmin, async (c) => {
	await c.env.DB.prepare("DELETE FROM inbox_members WHERE inbox_id = ? AND user_id = ?")
