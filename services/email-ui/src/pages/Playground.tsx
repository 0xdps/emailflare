...
function SyntaxCode({ code, lang, className = "" }: { code: string; lang: "html" | "json"; className?: string }) {
	...
	return (
		<pre
			className={`hljs rounded-lg text-xs font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed border border-border p-4 ${className}`}
		>
...
export default function PlaygroundPage() {
	...
	const keyMode: "test" | "live" | null = apiKey.startsWith("eftest_")
		? "test"
		: apiKey.startsWith("eflive_") || apiKey.startsWith("emailflair_")
			? "live"
			: null;

	useEffect(() => {
		Promise.all([api.get<Template[]>("/api/templates"), api.get<Domain[]>("/api/domains")]).then(
			([{ data: tpl }, { data: dom }]) => {
				setTemplates(tpl);
	...
	useEffect(() => {
		...
		if (selectedTemplate.layout) {
			api.post<{ html: string; subject: string }>(`/api/templates/${selectedTemplate.id}/preview`, {
				variables,
				themeId,
			})
				.then((r) => {
		...
		const obj: Record<string, unknown> = {
			...
			variables:
				Object.keys(variables).length > 0
					? variables
					: detectedVars.reduce((acc, v) => ({ ...acc, [v]: `<${v}>` }), {}),
		};
		...
		try {
			...
			const detail =
				typeof raw === "string"
					...
					: raw != null
						? JSON.stringify(raw)
						: `HTTP ${e2.response?.status ?? "unknown"}`;
							...
							<Select value={selectedId} onValueChange={(v) => v !== null && handleSelect(v)}>
								...
								<SelectContent>
									{filteredSystemTemplates.length > 0 && (
										<SelectGroup>
											<SelectLabel>
												Built-in templates ({filteredSystemTemplates.length})
											</SelectLabel>
									...
									{filteredCustomTemplates.length > 0 && (
										<SelectGroup>
											<SelectLabel>
												Custom templates ({filteredCustomTemplates.length})
											</SelectLabel>
							...
							{selectedTemplate && (
								<div className="flex items-center gap-1.5 flex-wrap">
									{selectedTemplate.is_system === 1 && (
										<Badge
											variant="secondary"
											className="text-[10px] text-purple-400 bg-purple-500/10 uppercase"
										>
											built-in
										</Badge>
									)}
									<p className="text-xs text-muted-foreground truncate">
										Subject: {previewSubject || selectedTemplate.subject}
									</p>
								</div>
							)}
						</div>
						...
						>
							...
							<div className="flex flex-col gap-1.5">
								...
								<div className="flex gap-2 items-center">
									<Input
										...
										placeholder="eftest_… or eflive_…"
										autoComplete="off"
										className="flex-1"
									/>
									{keyMode === "test" && (
										<Badge className="text-[10px] bg-amber-500/10 text-amber-600 border-amber-200 whitespace-nowrap">
											Test mode
										</Badge>
									)}
									{keyMode === "live" && (
										<Badge className="text-[10px] bg-emerald-500/10 text-emerald-600 border-emerald-200 whitespace-nowrap">
											Live mode
										</Badge>
									)}
								</div>
							...
							<div className="flex flex-col gap-1.5">
								...
								<div className="flex gap-1.5 items-center">
									...
									{domains.length > 0 ? (
										<Select
											value={fromDomain}
											onValueChange={(v) => v !== null && setFromDomain(v)}
										>
							...
							<div className="flex flex-col gap-1.5">
								<Label className="text-xs">
									Subject{" "}
									{selectedTemplate && (
										<span className="text-muted-foreground font-normal">(from template)</span>
									)}
								</Label>
								<Input
									value={subject}
										...
										selectedTemplate.layout ? (
											<>
												<div className="flex items-center gap-2">
													<Badge
														variant="secondary"
														className="text-[10px] text-purple-400 bg-purple-500/20 uppercase"
													>
														built-in
													</Badge>
													<p className="text-xs text-muted-foreground">
														Rendered server-side via React Email
													</p>
												</div>
												{detectedVars.length > 0 && (
													<div>
											...
											<>
												<div>
													...
													<pre className="bg-card border border-border rounded-xl px-4 py-3 text-xs text-amber-300 font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed">
														{previewSubject || selectedTemplate.subject}
													</pre>
												</div>
												<div>
													<p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-1.5">
												...
												{selectedTemplate.text_body && (
													<div>
														...
														<pre className="bg-card border border-border rounded-xl p-4 text-xs text-muted-foreground font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed">
															{selectedTemplate.text_body}
														</pre>
													</div>
												)}
							...
							<ScrollArea className="h-full">
								<div className="p-5 flex flex-col gap-5">
									...
									<div>
										...
										<pre className="bg-muted/50 border border-border rounded-lg p-4 text-xs text-foreground font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed">
											{curlSnippet}
										</pre>
									</div>
									<div>
										...
										<div className="bg-card border border-border rounded-xl divide-y divide-border">
											{(
												[
													...
													selectedTemplate
														? {
																field: selectedTemplate.slug
																	? "templateSlug"
																	: "templateId",
																value: selectedTemplate.slug ?? selectedTemplate.id,
															}
														: null,
													{ field: "themeId", value: themeId },
													...detectedVars.map((v) => ({
														field: `variables.${v}`,
														value: variables[v] || null,
													})),
												].filter(Boolean) as Array<{ field: string; value: string | null }>
											).map(({ field, value }) => (
												<div
													key={field}
												...
												>
													...
													<span
														className={
															value
																? "text-foreground font-mono max-w-xs truncate text-right"
