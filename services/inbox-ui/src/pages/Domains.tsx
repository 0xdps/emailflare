				...
				<Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>
								This will remove the domain and revoke all associated API key bindings. This cannot be
								undone.
							</DialogDescription>
						</DialogHeader>
						<DialogFooter>
						...
						{dnsModal?.records.map((r, i) => (
							<div key={i} className="bg-muted border border-border rounded-lg p-3 font-mono text-xs">
								<div className="flex gap-3 flex-wrap items-start">
									<Badge
										variant="outline"
										className="text-primary font-semibold w-10 flex-shrink-0 justify-center"
									>
										{r.type}
									</Badge>
