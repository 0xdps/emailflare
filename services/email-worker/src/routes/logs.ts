...
app.get("/", async (c) => {
	...
	const [dataResult, countResult] = await Promise.all([
		db.query(`SELECT * FROM email_logs ${where} ORDER BY sent_at DESC LIMIT ? OFFSET ?`, [
			...params,
			limit,
			offset,
		]),
		db.query<{ total: number }>(`SELECT COUNT(*) as total FROM email_logs ${where}`, params),
	]);
...
app.get("/:id", async (c) => {
	const db = new D1Db(c.env.DB);
	const row = await c.env.DB.prepare("SELECT * FROM email_logs WHERE id = ? LIMIT 1").bind(c.req.param("id")).first();
