...
app.post("/", zValidator("json", domainCreateSchema), async (c) => {
	...
	try {
		...
		cfResult =
			allSubdomains.find((s) => s.name === name) ??
			(await createSendingSubdomain(zoneId, name, env.CF_API_TOKEN));
	} catch (error) {
	...
	if (zoneId) {
		getBounceWorkerInfo(env.BOUNCE_WORKER_NAME, env.CF_API_TOKEN, env.CF_ACCOUNT_ID)
			.then(async (info) => {
				if (!info.deployed) return;
				await enableEmailRouting(zoneId, env.CF_API_TOKEN);
				await setCatchAllToWorker(
					zoneId,
					env.BOUNCE_WORKER_NAME,
					env.CF_API_TOKEN,
					"Bounce forwarding (emailflare)",
				);
			})
			.catch((err) => {
