...
webhooksRoutes.post("/bounce", async (c) => {
	...
	if (isComplaint(email)) {
		...
		if (existing.rows.length > 0) {
			await db.exec(`UPDATE email_logs SET status = 'complained', bounced_at = ? WHERE id = ?`, [
				now,
       ...
       VALUES (?, ?, 'complaint', NULL, ?, ?)`,
			[nanoid(), recipient, (existing.rows[0] as { id: string } | undefined)?.id ?? null, now],
		);
	...
	if (isBounce(email)) {
		...
		if (logId) {
			await db.exec(`UPDATE email_logs SET status = 'bounced', error = ?, bounced_at = ? WHERE id = ?`, [
				reason,
				now,
				logId,
			]);
