...
function printCf(label, result) {
  ...
  if (j && typeof j === 'object') {
    ...
    if (!success) {
      ...
      if (j.messages?.length) console.log(`   ${c.dim('messages:')} ${j.messages.map(m => m.message ?? m).join('; ')}`);
    }
    // Print result info (zones list, catch-all rule, etc.)
    if (j.result && typeof j.result === 'object') {
      if (Array.isArray(j.result)) {
        console.log(`   ${c.dim('result:')} ${j.result.map(r => r.name ?? r.id ?? JSON.stringify(r)).join(', ') || '(empty)'}`);
      } else {
...
async function verifyAccount() {
  ...
  if (!CF_ACCOUNT_ID) {
    ...
    if (r.json?.success && r.json?.result?.length) {
      console.log(`   ${c.green('✓')} Use one of these as cf_account_id: ${r.json.result.map(a => `${a.name} (${a.id})`).join(', ')}`);
    }
    return;
  }
...
async function findZone(domain) {
  ...
  if (perms.length === 0) {
    ...
    console.log('');
    if (missing.length) {
      console.log(`   ${c.yellow('⚠')} Missing permission(s) likely needed for Email Routing: ${missing.join(', ')}`);
      console.log(`      ${c.yellow('  → POST /email/routing/enable needs Zone Settings: Edit (#zone_settings:edit).')}`);
      console.log(`      ${c.yellow('  → PUT /email/routing/rules/catch_all needs Email Routing Rules: Edit (account-level).')}`);
      console.log(`      ${c.yellow('  → Add them to the token, then copy the NEW token value into scripts/config.toml (cf_api_token).')}`);
    } else {
