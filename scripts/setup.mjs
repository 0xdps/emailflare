...
if (d1IdMatch) {
	...
	if (!listMatch)
		die(`D1 database "${DB_NAME}" exists but could not read its ID.\nwrangler d1 list output:\n${d1List.stdout}`);
	d1Id = listMatch[1];
...
const jsonc = template.replace(/REPLACE_WITH_D1_DATABASE_ID/g, d1Id).replace(/REPLACE_WITH_KV_NAMESPACE_ID/g, kvId);
...
const SECRETS = [
	...
	{
		name: "ADMIN_ORIGIN",
		label: "ADMIN_ORIGIN (worker URL, e.g. https://emailflare-api-worker.ACCOUNT.workers.dev)",
		cfgVal: cfg.secrets?.admin_origin,
	},
